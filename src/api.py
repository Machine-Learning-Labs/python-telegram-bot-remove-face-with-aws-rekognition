...
def lambda_handler(event, context):
    ...
    metadata.put( Body=(bytes(json.dumps(event).encode('UTF-8'))), StorageClass='REDUCED_REDUNDANCY')
    ...
    # Call DetectFaces
    response = client.detect_faces(Image={'S3Object': {'Bucket': bucket_name, 'Name': f'{session_uuid}/{original_filename}'}}, Attributes=['DEFAULT'])

    # generate reference photo
    reference = generate_reference(image=image,
    ...
    for face in response['FaceDetails']:
        ...
        particular = generate_own(image=duplicate,
                                  output_path=output_path,
                                  extension=file_extension,
                                  id=id,
                                  faces=faces,
    ...
    for output in image_list:
        ...
        result = generated.put(Body=open(f'{output_path}/{output}', 'rb'), StorageClass='REDUCED_REDUNDANCY')

        url_signed = s3_client.generate_presigned_url('get_object', Params={'Bucket': bucket_name, 'Key': object_name}, ExpiresIn=3600)
...
def save_image(*, stream, path, name, footer_text):
    ...
    draw.text((border_size + w - text_width, h + border_size - 2), footer, font_color, font=font)
    img_with_border.save('{}/{}'.format(path, name))


def generate_reference(*, image, output_path, extension, response, imgWidth, imgHeight, footer_text):
    """
    ...
    for faceDetail in response['FaceDetails']:
        ...
        draw.ellipse([left, top, left + width, top + height], outline=reference_color, fill=TINT_COLOR + (OPACITY,))
        font = ImageFont.truetype(font_file, int(height*0.3))
        draw.text((left-width*0.1, top-height*0.4), str(counter), reference_color, font)
...
def generate_own(*, image, output_path, extension, id, faces, imgWidth, imgHeight, footer_text):
    """
    ...
    save_image(stream=image, path=output_path, name='{}{}'.format(id, extension), footer_text=footer_text)
